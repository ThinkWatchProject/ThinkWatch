...
// weight from the `models` table.
//
//   weighted = round(input × input_weight + output × output_weight)
//
...
impl WeightCache {
    ...
    pub async fn get(&self, pool: &PgPool, model_id: &str) -> Weights {
        ...
        let weights = match sqlx::query_as::<_, (rust_decimal::Decimal, rust_decimal::Decimal)>(
            "SELECT input_weight, output_weight FROM models WHERE model_id = $1",
        )
        .bind(model_id)
        .fetch_optional(pool)
        .await
...
pub fn weighted_tokens(input_tokens: i64, output_tokens: i64, mult: Weights) -> i64 {
