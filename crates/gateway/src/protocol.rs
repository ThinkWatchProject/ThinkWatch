...
//!
//! Lives here rather than in core: the strings are what
//! `model_routes.upstream_protocol` stores, and the candidate order is
//! keyed on this gateway's `provider_type` values.
//!
...
impl UpstreamProtocol {
    /// The conversion layer's name for this wire format.
    pub fn dialect(self) -> tw_dialect::ir::Dialect {
        use tw_dialect::ir::Dialect;
        match self {
            Self::OpenAiChat => Dialect::Chat,
            Self::OpenAiResponses => Dialect::Responses,
            Self::AnthropicMessages => Dialect::Anthropic,
            Self::GoogleGenerate => Dialect::Gemini,
            Self::BedrockNative => Dialect::Bedrock,
        }
    }
}
