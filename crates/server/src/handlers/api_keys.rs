...
/// Surface kinds accepted by `api_keys.surfaces`. Kept in lockstep
/// with the `api_key_surface_kinds` lookup table seeded by
/// `db/seeds.sql` and with `RateLimitSubject::Surface` on the limits
/// engine — the DB trigger is the final gate, this is the fast-path
/// check so handlers don't need a round-trip to reject a typo.
/// Adding a surface means editing this const AND the seed AND the
/// limits-engine enum.
