...
/// Result of a one-shot JSON-RPC `tools/list` probe against an MCP
/// endpoint. Used by both the manual `/test` endpoint (server edit
/// form) and `install_template` (auto-validation before persisting a
/// store install). Never persists anything.
