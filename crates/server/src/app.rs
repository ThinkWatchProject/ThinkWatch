...
pub async fn create_gateway_app(
    ...
    if let Err(e) = load_mcp_servers_into_registry(&state, &registry).await {
    ...
    spawn_mcp_health_loop(state.clone(), registry.clone(), pool.clone(), 60);
...
// ---------------------------------------------------------------------------
// MCP server loading from database
// ---------------------------------------------------------------------------
...
/// the cached metadata.
async fn load_mcp_servers_into_registry(
    state: &AppState,
