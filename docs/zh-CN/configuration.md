...
本文档提供 AgentBastion 所有配置选项的完整参考。配置通过环境变量（用于基础设施和密钥）和数据库存储的动态设置（可通过管理员 Web UI 配置）相结合进行管理。
...
用于 HS256 JWT 签名和验证的共享密钥。必须至少 32 个字符长，建议至少为 256 位（32 字节 / 64 个十六进制字符）以确保足够的安全性。启动时，AgentBastion 会对此值执行熵检查，如果不满足最低长度要求将拒绝启动。
...
Quickwit 实例的基础 URL，用于审计日志存储和搜索。审计日志**仅**存储在 Quickwit 中（不存储在 PostgreSQL 中）。如果未设置，审计日志的写入和搜索将不可用（条目仍会记录到标准输出并转发至已配置的日志转发器）。
...
存储审计日志条目的 Quickwit 索引名称。AgentBastion 会在启动时自动创建索引（如果不存在）。
...
| `site_name`            | `AgentBastion`   | Web UI 中显示的站点名称                  |

---

## 启动验证

AgentBastion 在启动时验证所有配置和依赖，如果出现以下情况将拒绝启动：
...
- **Quickwit：** 如已配置，验证索引是否存在（非阻塞 -- 不可用时记录警告）
