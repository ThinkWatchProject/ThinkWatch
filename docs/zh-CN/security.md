...
由 `crates/common/src/validation.rs` 中的 `validate_password()` 函数校验。
...
所有日志查询端点使用 `?` 占位符的参数化查询，通过 ClickHouse 客户端 SDK 绑定参数。用户输入的 LIKE 搜索内容会转义 `%`、`_`、`\` 字符，防止通配符注入。

排序字段使用白名单 `match` 表达式和安全默认值回退，防止通过 ORDER BY 子句的 SQL 注入。
