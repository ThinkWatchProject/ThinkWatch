...
本文档描述了 AgentBastion 的安全架构、机制和加固实践。

---

## 1. 认证

AgentBastion 支持三种认证机制。网关接受其中任意一种；控制台管理 API 需要 JWT。

### 1.1 JWT 令牌

AgentBastion 在认证成功后签发两个 JWT 令牌：
...
- 启动时，AgentBastion 执行熵检查，拒绝明显弱的密钥（例如全部相同字符、常见模式）。
...
AgentBastion 支持 OpenID Connect，用于与企业身份提供商（Entra ID、Okta、Keycloak、Auth0 等）进行单点登录。
...
1. 如果 `Authorization` 请求头包含以 `ab-` 开头的令牌，则将其作为 API 密钥验证。
...
AgentBastion 实现了基于角色的访问控制，包含五个系统角色。
...
AgentBastion 本身不终止 TLS。TLS 终止应由反向代理（Nginx、Caddy、Traefik、云负载均衡器）处理。详见第 6 节的加固清单。
...
审计条目被索引到 Quickwit 中，用于全文搜索和分析：

- 索引：可通过 `QUICKWIT_INDEX` 配置（默认：`agentbastion-audit`）
- 条目异步发送，以避免阻塞请求处理。
...
AgentBastion 在启动服务器前验证所有密钥和依赖。如果任何关键要求未满足，进程将以清晰的错误消息退出，而不是在降级状态下运行。
...
在准备 AgentBastion 的生产部署时，请使用此清单。
...
- [ ] 将 PostgreSQL 访问限制为仅 AgentBastion 服务账户
